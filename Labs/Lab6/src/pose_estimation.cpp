...
void evaluateRPE(const tf::Transform& gt_t_prev_frame,
  ...
  // 2D-2D (pose_estimator < 3): 单目没有尺度，需要将两者都归一化为单位向量，这样比较的是“运动方向”的误差
  // 同时只有当两个向量都有长度时才归一化，防止除以0
  //
  // 3D-3D (pose_estimator = 3): 比较绝对距离不用归一化
  ...
  // 计算旋转误差：计算两个旋转之间的差异角度
  // diff_q 代表从 est 到 gt 需要转多少度
  tf::Quaternion q_diff = q_gt.inverse() * q_est;
  double rotation_error_rad = q_diff.getAngle(); // 结果是弧度
  double rotation_error_deg = rotation_error_rad * 180.0 / M_PI; // 转为角度

  // 3. 打印误差到终端 (可选，方便实时看)
  // std::cout << "RPE Trans: " << translation_error << " | RPE Rot (deg): " << rotation_error_deg << std::endl;

  // 4. 将误差写入文件
  // 使用 static 保持文件流开启
  static std::ofstream log_file;
  
  // 只有第一次调用时（文件未打开时）执行打开操作
  if (!log_file.is_open()) {
      std::string output_path = "/home/stanley/vnav_ws/src/lab6/log/"; // 你的代码目录
      std::string filename;
      ...
      // 打开文件，std::ios::trunc 表示如果文件存在则清空内容重新写
  ...
  case 0: {
    ...
    if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac) {
        ...
        // 【关键点】设为1次迭代 = 随机取5个点计算，不剔除外点
        ransac.max_iterations_ = 1; 
        ransac.threshold_ = 100000.0; // 阈值设大，保证能算出结果
        ...
        // 3. 设置参数 (阈值对应 bearing vector 的角度误差，这里设为约 0.5 像素的误差)
        // 1.0 - cos(theta), theta ~= 1.0 / 800.0
        ...
        // 5. 转换结果：OpenGV 返回的是 Eigen 矩阵，转换为 ROS Pose
        // model_coefficients 是一个 3x4 的矩阵 [R|t]
        relative_pose_estimate = eigen2Pose(ransac.model_coefficients_);
  ...
  case 3: {
    ...
    if (adapter_3d.getNumberCorrespondences() >= min_nr_of_correspondences) {
      if (!FLAGS_use_ransac){
        ...
        // 3. Trick: 只迭代 1 次，模拟无 RANSAC 的效果 (Least Squares)
        ransac.max_iterations_ = 1;
        ransac.threshold_ = 1000.0; // 阈值极大，接受所有点
        ...
        // 3. 设置参数
        // 注意：3D-3D 的阈值单位是米(meters)。
        // 0.1 表示如果两个 3D 点重投影后的距离小于 10cm 则认为是内点
