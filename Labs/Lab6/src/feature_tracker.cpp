...
void FeatureTracker::trackFeatures(const cv::Mat &img_1,
  ...
  cv::TickMeter tm_detect, tm_match;
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  // ~~~~ begin solution
  cv::Mat img_inliers;
  // 将 uchar mask 转换为 drawMatches 需要的 vector<char>
  ...
  cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, good_matches, img_inliers,
                  cv::Scalar(0, 255, 0), // Inliers 为绿色
                  cv::Scalar(0, 0, 255), // 这里的颜色参数略微复杂，OpenCV默认行为即可
                  mask_char,
                  cv::DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);
  ...
  // 将匹配好的特征点填充到输出变量中，传给 pose_estimation.cpp
  matched_kp_1_kp_2->first.clear();
  matched_kp_1_kp_2->second.clear();
  for (const auto& match : good_matches) {
      matched_kp_1_kp_2->first.push_back(keypoints_1[match.queryIdx]);
      matched_kp_1_kp_2->second.push_back(keypoints_2[match.trainIdx]);
  }
