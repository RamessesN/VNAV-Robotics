...
*** *** *** *** ***
    ...
    \textit{Other notes:} I found that different $OS$ will produce different random sequences even with the same seed value.
    At first, I suspected that the discrepancy was caused by the compiler. On macOS, the native C++ compiler is \textbf{Clang}, whereas on Ubuntu it is \textbf{GCC}. 
    However, after installing Clang on Ubuntu and recompiling the code, the generated random sequence remained identical to that produced by GCC.
    Therefore, I concluded that the variation in random sequences arises from differences in the underlying C standard library implementations across operating systems, 
    rather than from the compiler itself. Specifically, the \texttt{rand()} function may employ distinct algorithms or entropy sources on different platforms, resulting in divergent random outputs.
...
By combining Linux command-line tools, Git-based project management, and C++ programming, I not only strengthened my foundational programming skills but also gained valuable experience in practical robotics system development and experimental workflows.
\newpage
